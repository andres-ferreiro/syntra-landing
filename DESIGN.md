...
- `components/ui/`: `Container`, `CtaButton` (primary / secondary / inverted variants — a `<Link>`, for navigation), `SectionHeader` (headline + optional subcopy, no eyebrow), `Reveal` (IntersectionObserver scroll-fade, respects `prefers-reduced-motion`), `Logo`; plus a small set of shadcn-style primitives for the phone input specifically — `button.tsx` (a real `<button>`, distinct from `CtaButton`), `input.tsx`, `popover.tsx`, `command.tsx`, `scroll-area.tsx`, `drawer.tsx` (thin wrappers around Radix/`cmdk`/`vaul`, restyled onto this project's tokens, not shadcn's default `bg-background`/`ring` classes), and `phone-input.tsx` (see `ContactSection` below). The country combobox renders in a `Popover` at `md`+ and a bottom `Drawer` below it (`lib/use-is-mobile.ts`) — a small anchored flyout is a poor touch target on a phone, where a full-width sheet is the native-feeling pattern. All of `ContactSection`'s text fields use `text-base sm:text-sm` rather than a flat `text-sm`, since iOS Safari zooms the page on focusing any input under 16px.
...
- `components/patterns/`: `ServiceCard` (icon + title + description, no decorative numbering — see `ServiceIcon`), `ServiceIcon` (hand-drawn, one per service, keyed by title), `IndustryCard` (pill, not boxed — intentionally different from `ServiceCard`), `FeatureGrid` (checklist), `IntegrationRow` (channel pills with hand-drawn icons), `FaqAccordion` (native `<details>`), `ChatMockup`, `CalendarMockup`, `CrmMockup` (kanban-style lead pipeline, `layout`-animated card moves on hover), `FunnelMockup`, `ContactFlowPreview` (white card: two chat bubbles + a collapsible "used N actions" log, echoing modern AI-agent tool-call disclosure UI, floating over `DotField`, `/contact` only — see Motion below; each of its 4 actions gets a distinct rounded-square icon badge rather than one repeated shape — teal for Calendar (on-brand), violet for CRM (matching `CrmMockup`'s stage-dot violet), blue for the team-notify bell, and the site's real WhatsApp logo asset for that one action, not a hand-drawn approximation), `DotField` (canvas-drawn dot grid that bulges toward the cursor on hover, no separate glow shape; ported from react-bits' `DotField-JS-CSS` registry component and retinted to blue-only-as-accent rather than its default dark gradient; static, non-interactive grid under reduced motion), `ContactSection` (owns the whole `/contact` right column, not just the form: headline + secondary "book a call" CTA + the full 15-field qualification form (first/last name collected separately so the post-submit calendar's required last-name field always prefills correctly; phone uses `components/ui/phone-input.tsx` — the shadcn-phone-input pattern (github.com/omeralpi/shadcn-phone-input) adapted onto this project's own tokens/primitives rather than shadcn's default `bg-background`/`text-foreground` classes, which aren't defined here: a searchable country combobox (`Popover`+`Command`+`ScrollArea`, all in `components/ui/`) with real flags, backed by `react-phone-number-input`/`libphonenumber-js`, defaulting to Mexico on `/es` and the US on `/en`, validated with `isValidPhoneNumber` before advancing past step one — the CRM API always receives a real, verified E.164 string rather than an ambiguous local number), split into 3 onboarding-style steps, posting to `/api/contact` — then, on success, swaps to a compact confirmation with an explicitly-opt-in "Opcional" booking card, and if that's chosen, swaps again to just a header + `CalendarEmbed` filling the column, nothing else competing for space), `DirectContactLinks` (plain underlined text links, not pills — email/phone/WhatsApp, `/contact` only), `CalendarEmbed` (iframe booking widget with prefill, used on `/schedule` and `ContactSection`'s post-submit step), `TestimonialPlaceholder` (built, intentionally unused — see Content Rules).
