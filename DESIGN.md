...
- `components/ui/`: `Container`, `CtaButton` (primary / secondary / inverted variants — a `<Link>`, for navigation), `SectionHeader` (headline + optional subcopy, no eyebrow), `Reveal` (IntersectionObserver scroll-fade, respects `prefers-reduced-motion`), `Logo`; plus a small set of shadcn-style primitives for the phone input specifically — `button.tsx` (a real `<button>`, distinct from `CtaButton`), `input.tsx`, `popover.tsx`, `command.tsx`, `scroll-area.tsx` (thin wrappers around Radix/`cmdk`, restyled onto this project's tokens, not shadcn's default `bg-background`/`ring` classes), and `phone-input.tsx` (see `ContactSection` below).
