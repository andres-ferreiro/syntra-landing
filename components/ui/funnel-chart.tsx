...
// Vendored third-party component (marketing "funnel chart" primitive),
// following the same precedent as globe.tsx / orbital-clock.tsx / animated-beam.tsx.
// Two adaptations for this project, everything else is upstream:
//   1. shadcn's default semantic classes (text-foreground, bg-muted, etc.)
//      are remapped to this project's own tokens (ink, ink-soft, border,
//      paper) since this codebase doesn't use the shadcn palette.
//   2. Default colors point at the brand's accent-strong → accent → mint
//      family instead of a generic --chart-1..5 rainbow, per DESIGN.md's
//      "one restrained accent color" rule.
...
function SegmentLabel({
  ...
  const valueEl = showValues && (
    <span className="whitespace-nowrap font-semibold text-ink text-sm">
      {display}
    </span>
  );
  const pctEl = showPercentage && (
    <span className="rounded-pill bg-ink px-3 py-1 font-bold text-paper text-xs shadow-sm">
      {formatPercentage(pct)}
    </span>
  );
  const labelEl = showLabels && (
    <span className="whitespace-nowrap font-medium text-ink-soft text-xs">
      {stage.label}
    </span>
  );
...
export function FunnelChart({
  ...
  const bandColor = gridCfg.bandColor ?? "var(--color-border)";
